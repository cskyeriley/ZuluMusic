...
var bc_links = new Array(), bc_names = new Array();
...
window.onload = function initPage() {
    try {
        mpmetrics = new MixpanelLib("5fcc53980ee8e2260b04ed6b64bcd51b");
    } catch (err) {
        var null_fn = function () {};
        mpmetrics = { track: null_fn, track_funnel: null_fn,
...
function getAlbumInfo(id) {
    ...
    mpmetrics.track('album_detail', {'id': id});
    state.album.id = id;
    bt.stash.set("state", JSON.stringify(state));
...
function getArtistInfo(id) {
    ...
    mpmetrics.track("artist_detail", {"id": id});
    state.artist.id = id;
	$("#gallery, #albums, #searchWrapper").empty();
	displayBreadcrumbs();
		...
		success: function(xml3) {
			...
			$(xml3).find('artist').each(function(){
				...
				if (!element) {
					...
					var mp3URL = "http://imgjam.com/torrents/album/"+specialDownload(albumID)+"/"+albumID+"/"+albumID+"-mp32.torrent"
					var oggURL = "http://imgjam.com/torrents/album/"+secretNumber+"/"+id+"/"+id+"-ogg3.torrent"
					/* albumsMP3[k] = mp3URL;
					albumsOGG[k] = oggURL;
					k++; */
					...
					$("<div id='dl_"+albumID+"'></div>").appendTo("#item_"+albumID);
					$("<img class='mainDL' src='img/dl_lg.png'/>").insertBefore("#title_"+albumID).fadeTo(1, 0.5);
					var widget = new bt.Widget.Download({
						name      : title+" (MP3)",
						url       : mp3URL,
						elem      : $("#dl_"+albumID)[0],
					...
					$("#item_"+albumID+" img.mainDL").click(function() {
						$(this).attr("src", $(this).attr("src").replace("lg", "load"));
						bt.add.torrent(mp3URL, function() {
							widget.settings.callbacks.dispatch('addTorrent');
						});
					});
					$("#item_"+albumID).hover(function(){
						if($("img.mainDL", this).is(":visible")){
							$("img.mainDL", $(this)).fadeTo(500, 1);
						}
					},
...
function renderFooter () {
    ...
    $("#ZULUtv").click(function(){location.href="http://github.com/downloads/stevenviola/ZuluTV/Zulu%20TV.btapp";})
}
