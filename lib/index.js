		...
		success: function(rss) {
			$(rss).find('album').each(function(){
				...
				$("#artist_"+artistID).click(function() {
					bc_links[2] = "getArtistInfo('"+artistID+"','"+artistName+"')"; 
					bc_names[2] = artistName;
					getArtistInfo(artistID,artistName);
				});
			});
		}
	});
...
function getAlbumInfo(id) {
	...
	$.ajax({
		type: "GET",
		url: "http://api.jamendo.com/get2/image+name+artist_name+artist_id/album/xml/?album_id="+id+"&imagesize=200",
		dataType: "xml",
		success: function(xml) {
			$(xml).find('album').each(function(){
				...
				bc_links[2] = "getArtistInfo('"+artistID+","+artist+"')";
				bc_names[2] = artist;
...
function getArtistInfo(id,artist) {
	$("#gallery").empty();
	$("#albums").empty();
	$("#searchWrapper").empty();
	displayBreadcrumbs();
	...
	$("<div id='albums'></div>").appendTo('#albums');
	$.ajax({
		type: "GET",
		url: "http://api.jamendo.com/get2/id+album_name+album_id+artist_id+image/artist/xml/track_album+album_artist/?artist_id="+id+"&imagesize=200",
		dataType: "xml",
		success: function(xml3) {
			$(xml3).find('artist').each(function(){
				...
				if (!element) {
					...
					$("<div id='artistInfo' class='artistTitle'>"+bc_names[2]+"</div>").appendTo("#artistHeading");
					/*
			...
			$(xml3).find('artist').each(function(){
				...
				if (!element) {
					...
					albumsOGG[k] = oggURL;
					k++;
					$("<li class='items' id='item_"+albumID+"'></li>").html("<img id='poster_"+albumID+"' class='poster link' title='"+albumID+"' onerror='onImgError(this)' src='"+imageURL+"'/>").appendTo('#gallery');
...
function renderFooter () {
	$("#footer").append("<div href='http://stevenviola.com/zulu/Zulu.btapp' id='ZULUtv' class='center link'>Want to download your favorite TV Shows? Try Zulu TV</div>");
}
