...
window.onload = function initPage() {
    _.each(bt.torrent.keys(), function(key){
        console.log(bt.torrent.get(key));
    });
	...
	$("<ul id='gallery' class='center'></ul>").appendTo('#main');
...
function getAlbumLicense(id) {
	$.ajax({
		...
		success: function(resp) {
            ...
            console.log(imgurl+"88x31.png");
            if(imgurl){
                $("<div><a href='"+resp[0].url+"'><img src='"+imgurl+"88x31.png"+"' title='"+alt+"'/></a></div>").insertAfter("#artist");
            } else {
                $("<div><a href='"+resp[0].url+"'>"+alt+"</a></div>").insertAfter("#artist");
            }
		}
	});
}
